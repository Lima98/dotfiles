...
// [0, 1]
#define GLOW_COLOR_OFFSET_BRIGHTNESS 0.35
// Time slowdown factor for animations
// x \in R : x > 0
#define TIME_DURATION_FACTOR 0.5
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Detect highlight pixels
    float highlightMask = 1.0 - smoothstep(
            0.0,
            HIGHLIGHT_THRESHOLD,
            distance(fragColor.rgb, HIGHLIGHT_COLOR)
            );

    if (highlightMask > 0.0) {
        // Compute distance to center of the highlight pixel
        // Since we only have per-pixel info, approximate glow using smooth radial mask
        // We use uv-space offsets for a simple falloff
        float glow = 0.0;
        for (int dx = -2; dx <= 2; dx++) {
            for (int dy = -2; dy <= 2; dy++) {
                float dist = length(vec2(dx, dy)) / 2.828427; // normalize to [0,1]
                float weight = exp(-dist * dist * 3.0);       // Gaussian falloff
                glow += weight;
            }
        }
        glow /= 25.0; // normalize

        // Apply highlight color as glow, multiply by mask and strength
        fragColor.rgb += HIGHLIGHT_COLOR * glow * highlightMask * GLOW_STRENGTH;
    }
